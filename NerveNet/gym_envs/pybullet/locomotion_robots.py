...
assets_directory = Path("NerveNet/gym_envs/assets")


class MJCFBasedRobot(robot_bases.MJCFBasedRobot):
    '''
    ...
    To create new environments based on pybullets MJCFBasedRobot implementation
    we needed to change the directory from where it loads the mjcf.xml files
    '''
    ...
    def reset(self, bullet_client):
        ...
        if (self.doneLoading == 0):
            ...
            if self.self_collision:
                self.objects = self._p.loadMJCF(os.path.join(assets_directory / self.model_xml),
                ...
                self.objects = self._p.loadMJCF(
                    os.path.join(assets_directory, self.model_xml, flags=pybullet.URDF_GOOGLEY_UNDEFINED_COLORS))
                self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(
                    self._p, self.objects)
...
class Ant_Crippled_BackLegs(WalkerBase):
    ...
    def __init__(self):
        WalkerBase.__init__(self, "cp_back_ant.xml", "torso",
                            action_dim=4, obs_dim=20, power=2.5)
...
class Ant_Crippled_LeftBackLeg(WalkerBase):
    """
    Copy of the Ant Pybullet Robot but without the actuators for the two back legs
    """
    foot_list = ['front_left_foot', 'front_right_foot',
                 'left_back_foot', 'right_back_foot']

    def __init__(self):
        WalkerBase.__init__(self, "cp_left_back_ant.xml", "torso",
                            action_dim=6, obs_dim=24, power=2.5)
...
class Ant_Crippled_RightBackLeg(WalkerBase):
    """
    Copy of the Ant Pybullet Robot but without the actuators for the two back legs
    """
    foot_list = ['front_left_foot', 'front_right_foot',
    ...
    def __init__(self):
        WalkerBase.__init__(self, "cp_right_back_ant.xml", "torso",
                            action_dim=6, obs_dim=24, power=2.5)
